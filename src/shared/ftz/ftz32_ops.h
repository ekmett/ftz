...
namespace ftz { namespace detail {
  ...
  native_nodiscard native_inline native_const unsigned int ftz32_trig_single(unsigned int bits) {
    ...
    if(magnitude<0x46000000u) {
      ...
      if constexpr(Cosine) return ::ftz::detail::math::fp32_encode(::ftz::detail::native::cos_ftz<Hardware>(x));
      else return ::ftz::detail::math::fp32_encode(::ftz::detail::native::sin_ftz<Hardware>(x));
    }
    auto reduced=ftz32_trig_reduce(magnitude);
    float x=::ftz::detail::math::fp32_decode(reduced.residual);
    bool odd=(reduced.quadrant&1u)!=0u;
    float value;
    if constexpr(Cosine) value=odd?::ftz::detail::native::sin_reduced_ftz<Hardware>(x):native::cos_reduced_ftz<Hardware>(x);
    else value=odd?::ftz::detail::native::cos_reduced_ftz<Hardware>(x):native::sin_reduced_ftz<Hardware>(x);

...
static_assert(!can_copy<::native::wide<ftz::m32,3>,::native::wide<ftz::h32,3>>);
template<class F,std::size_t L,std::size_t N>void vectors(){
 using V=::native::simd<F,L,arch>;using W=::native::wide<V,N>;using M=typename V::mask;
 static_assert(std::same_as<decltype(isfinite(W{})),::native::wide<M,N>>);
 static_assert(std::same_as<decltype(isinf(W{})),::native::wide<M,N>>);
 static_assert(std::same_as<decltype(isnan(W{})),::native::wide<M,N>>);
 static_assert(std::same_as<decltype(signbit(W{})),::native::wide<M,N>>);
 ...
 for(auto word:words){
  ...
  auto vector=::native::load_simd<V>(source.data());W x(vector),signs(V(F::from_bits(0x80000000u)));
  ...
  auto mask=[](M m,bool expected){
   ...
   else {std::array<std::uint32_t,L> bits{};::native::mask_bits<std::uint32_t>(m).store(bits.data());for(auto b:bits)check(b==(expected?0xffffffffu:0u));}
  };
  for(std::size_t i=0;i<N;++i){
   ...
   std::array<F,L> output{};::native::store_simd(output.data(),result.registers[i]);
   for(auto lane:output)check(lane.to_bits()==(magnitude|0x80000000u));
  }
 }
}
template<class F>void transport(){
 using V2=::native::simd<F,2,arch>;using V3=::native::simd<F,3,arch>;
 ...
 for(std::size_t i=0;i<words.size();++i){
  ...
  V3 value=::native::load_simd<V3>(input.data());
  V3 reversed=value.zyx;std::array<F,3> triple{};::native::store_simd(triple.data(),reversed);
  for(std::size_t j=0;j<3;++j)check(triple[j].to_bits()==input[2-j].to_bits());
  V2 pair=value.xy;pair.xy=pair.yx;std::array<F,2> lanes{};::native::store_simd(lanes.data(),pair);
  check(lanes[0].to_bits()==input[1].to_bits());check(lanes[1].to_bits()==input[0].to_bits());
  value.xy=value.yx;::native::store_simd(triple.data(),value);
...
template<class F>void run(){
 ...
 vectors<F,16,3>();
#endif
 for(auto word:words){auto value=F::from_bits(word);::native::wide<F,3> x(value),y(F::from_bits(0x80000000u));
  static_assert(std::same_as<decltype(isfinite(x)),::native::wide<bool,3>>);

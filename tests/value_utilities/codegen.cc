...
using V=simd::vec<ftz::m32,4,FTZ_TEST_ARCH>;
using R=simd::vec<float,4,FTZ_TEST_ARCH>;
#define CLASSIFY(Name) extern "C" [[gnu::noinline,gnu::flatten]] void check_##Name(std::uint32_t *o,float const *p){simd::mask_bits<std::uint32_t>(Name(V::unsafe_from_float32(R::loadu(p)))).store(o);}

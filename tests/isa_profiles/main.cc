...
int main(int argc, char ** argv) {
  ...
  {
    ftz::native_fp32_scope environment(ftz::native_fp32_mode::flush);
    if (!environment.controls_match() || !ftz::probe_ftz32_cpu().admitted()) return 3;
    unsigned active = request & admitted;
    if (active & 1u) {
      if (!run(profile_avx2, avx2)) return 4;
    }
    if (active & 2u) {
      if (!run(profile_avx512, avx512)) return 6;
    }
  ...
  if (argc > 3) {
    ...
    auto const & result = request & admitted & 1u ? avx2 : avx512;
    if (!compare(result, expected, "Original/current baseline")) return 15;
  }
  std::printf("profiles=%u words=%zu policy=%u exact\n", request & admitted,
    profile_test::words, unsigned(FTZ_FP32_HARDWARE_FTZ));
  return 0;
}

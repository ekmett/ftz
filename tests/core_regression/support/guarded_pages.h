...
namespace ftz::test {
  ...
  struct guarded_pages {
    guarded_pages() {
      ...
      if (size <= 0) ftz::test::fail(std::runtime_error("Unable to query page size"));
      page_size_ = static_cast<std::size_t>(size);
#endif
      if (page_size_ == 0 || page_size_ > std::numeric_limits<std::size_t>::max() / 3)
        ftz::test::fail(std::runtime_error("Invalid guarded allocation page size"));
#if defined(_WIN32)
      base_ = static_cast<std::byte *>(VirtualAlloc(nullptr, page_size_ * 3,
        MEM_RESERVE | MEM_COMMIT, PAGE_NOACCESS));
      if (!base_) ftz::test::fail(std::system_error(static_cast<int>(GetLastError()),
      ...
      if (!VirtualProtect(base_ + page_size_, page_size_, PAGE_READWRITE, &old_protection)) {
        ...
        ftz::test::fail(std::system_error(static_cast<int>(error), std::system_category(),
      ...
      if (mapping == MAP_FAILED) ftz::test::fail(std::system_error(errno, std::generic_category(),
        "mmap guarded pages"));
      ...
      if (mprotect(base_ + page_size_, page_size_, PROT_READ | PROT_WRITE) != 0) {
        ...
        ftz::test::fail(std::system_error(error, std::generic_category(), "mprotect middle page"));
      }
#endif
    }

...
namespace {
  ...
  void require(bool ok,char const * message) { if (!ok) ftz::test::fail(std::runtime_error(message)); }
  ...
  constexpr word reference(word x,word y,bool ftz,bool daz,bool output_flush,bool raw_before_rounding) {
    if (ftz || daz) { x=flush(x);y=flush(y); }
    // Typed FTZ treats every active NaN base as quiet, making its nonfinite
    // scaling behavior independent of the otherwise uncontracted payload.
    if (ftz && is_nan(x) && magnitude(y)==infinity) x=quiet_nan;
    ...
    return scale_integer(x,floor_exponent(y),ftz||output_flush,!ftz&&raw_before_rounding);
  }
  ...
  template<class V,class M> std::size_t check_mask(std::vector<row> const & rows,bool daz,bool output_flush,bool raw_before_rounding) {
    ...
    if constexpr(ftz) {
      static_assert(std::same_as<decltype(scaleb(std::declval<V>(),std::declval<V>())),V>);
      static_assert(std::same_as<decltype(masked_scaleb(std::declval<M>(),std::declval<V>(),std::declval<V>(),std::declval<V>())),V>);
      static_assert(std::same_as<decltype(masked_scaleb_zero(std::declval<M>(),std::declval<V>(),std::declval<V>())),V>);
    }
    ...
    for (std::size_t start=0;start<rows.size();start+=N) {
      ...
      for (std::size_t lane=0;lane<N;++lane) {
        ...
        x[lane]=ftz?flush(r.value):r.value;y[lane]=r.exponent;prior[lane]=ftz?flush(r.prior):r.prior;
      }
      ...
      auto inspect=[&](V result,std::uint64_t bits,bool zero) {
        ...
        for (std::size_t lane=0;lane<N;++lane) {
          ...
          word want=active?reference(x[lane],y[lane],ftz,daz,output_flush,raw_before_rounding):(zero?0:prior[lane]);
          bool equal=active?ftz::math_test::equivalent_fp32(actual[lane],want):actual[lane]==want;
          if (!equal) ftz::test::fail(std::runtime_error(std::string(ftz?"FTZ":"raw")+" scaleb N="+std::to_string(N)+
      ...
      for (auto bits:masks) {
        ...
        if constexpr(ftz) {
          auto typed_y=V::load_bits(y.data());
          inspect(masked_scaleb(mask,vp,vx,typed_y),bits,false);
          inspect(masked_scaleb_zero(mask,vx,typed_y),bits,true);
        }
      }
      inspect(scaleb(vx,vy),all_bits,false);
      if constexpr(ftz) {
        auto typed_y=V::load_bits(y.data());
        inspect(scaleb(vx,typed_y),all_bits,false);
      }
    }
    return checked;
  }
...
int main(int argc,char ** argv) {
  {
    ...
    bool check_ftz_profile=mode=="gradual"||mode=="flush",raw_before=profile=="before-rounding";
...
#else
      require(check_ftz_profile,"Separate DAZ/FTZ modes require x86 MXCSR");
#endif
      auto requested=ftz::read_native_fp_state().control;
      if(check_ftz_profile) {
        admitted=ftz::probe_ftz32_cpu().admitted();
...
#endif
#if defined(__AVX2__)
      checked+=check_width<8>(rows,daz,output_flush,raw_before,admitted);
#endif
#if defined(__AVX512F__) && defined(__AVX512DQ__)
      checked+=check_width<16>(rows,daz,output_flush,raw_before,admitted);
#endif
      require(ftz::read_native_fp_state().control==requested,"FP controls changed");
    }
    require(before==ftz::read_native_fp_state(),"FP environment not restored");
    std::cout<<"{\"passed\":true,\"mode\":\""<<mode<<"\",\"raw_profile\":\""<<profile
      <<"\",\"hardware_ftz\":"<<FTZ_FP32_HARDWARE_FTZ<<",\"rows\":"<<rows.size()<<",\"checked_words\":"<<checked
      <<",\"raw_daz\":"<<(daz?"true":"false")<<",\"raw_ftz\":"<<(output_flush?"true":"false")
      <<",\"ftz_profile_checked\":"<<(check_ftz_profile?"true":"false")<<",\"ftz_admitted\":"<<(admitted?"true":"false")<<",\"environment_restored\":true}\n";
  }
}
